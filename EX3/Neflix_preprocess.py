...
PICKLE_FILE_NAME_NETFLIX_MATRIX_EMPTY = 'netflix_matrix_empty'
PICKLE_FILE_NAME_MOVIES_INFO = 'movies_info.pkl'
PICKLE_FILE_NAME_MOVIES_INFO_genres = 'movies_info_genres.pkl'


def create_initial_data(remove_empty_cols=True, use_genres=False):
	"""
	...
	:return:
	"""
	FILE_NAME_NETFLIX_MATRIX = PICKLE_FILE_NAME_NETFLIX_MATRIX if not remove_empty_cols else PICKLE_FILE_NAME_NETFLIX_MATRIX_EMPTY
	PICKLE_FILE_NAME_MOVIES_INFO_NAME = PICKLE_FILE_NAME_MOVIES_INFO_genres if use_genres else PICKLE_FILE_NAME_MOVIES_INFO
	if not os.path.exists(PICKLE_FILE_NAME_NETFLIX_MATRIX+'.npz') or not os.path.exists(PICKLE_FILE_NAME_MOVIES_INFO):
		...
		with zipfile.ZipFile('netflx_prize_dataset.zip', 'r') as z:
			...
			save_created_files(df_of_movies_info, mat_of_movies_and_users, FILE_NAME_NETFLIX_MATRIX, PICKLE_FILE_NAME_MOVIES_INFO_NAME)
	else:
...
def load_files_from_disk():
	...
	mat_of_movies_and_users = scipy.sparse.load_npz("reducted_matrix_movies_and_users" + '.npz').tolil()
...
def save_created_files(df_of_movies_info, mat_of_movies_and_users, title_matrix, title_movies_info):
	"""
	...
	try:
		...
		scipy.sparse.save_npz(title_matrix, mat_of_movies_and_users.tocsr(), compressed=True)
		joblib.dump(df_of_movies_info, title_movies_info)
...
# mat_of_movies_and_users, df_of_movies_info = create_initial_data(remove_empty_cols=False, use_genres=True)